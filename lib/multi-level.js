...
class MultiLevelCache {
  ...
  async get(key, opts, updateFn) {
    ...
    const now = Date.now();
    if (item.expiry < now) {
      const maxStaleness = getValue(opts, 'maxStaleness', this._maxStaleness);
      if (maxStaleness !== Infinity && item.expiry + maxStaleness < now) {
  ...
  async _forceRefresh(key, staleItem, updateFn, opts) {
    try {
      ...
      const maxAge = getValue(opts, 'maxAge', this._maxAge);
      const cacheItem = {
        value,
        expiry: maxAge === Infinity ? maxAge : Date.now() + maxAge
      };
